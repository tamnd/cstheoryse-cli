...
describes what users get. Pick a format with `-o`, or let cst2 choose:
a table when writing to a terminal, JSONL when piped.

## Formats

```bash
cst2 <command> -o table   # aligned columns for reading
cst2 <command> -o jsonl   # one JSON object per line, for piping
cst2 <command> -o json    # a single JSON array
cst2 <command> -o csv     # spreadsheet friendly
cst2 <command> -o tsv     # tab-separated
cst2 <command> -o url     # just the URL column
cst2 <command> -o raw     # the underlying bytes, unformatted
```
...
```bash
cst2 <command> --fields id,title,url
```
...
```bash
cst2 <command> --template '{{.URL}} {{.Title}}'
```
...
```bash
cst2 <command>            # a table, because this is a terminal
cst2 <command> | wc -l    # JSONL, because this is a pipe
```
