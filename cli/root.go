// Package cli builds the cst2 command tree on top of the cstheoryse library.
package cli
...
type App struct {
	client *cstheoryse.Client
	cfg    cstheoryse.Config
...
func Root() *cobra.Command {
	app := &App{cfg: cstheoryse.DefaultConfig()}

	root := &cobra.Command{
		Use:   "cst2",
		Short: "Browse Computer Science Theory Stack Exchange",
		Long: `cst2 reads Computer Science Theory Stack Exchange through the public
Stack Exchange API (no key required). It returns questions as table, JSON,
JSONL, CSV, TSV, or URLs.

cst2 is an independent tool and is not affiliated with Stack Exchange or its
network of sites.`,
		SilenceUsage:  true,
		SilenceErrors: true,
...
func (a *App) setup() error {
	...
	a.client = cstheoryse.NewClient(a.cfg)
	return nil
}
